...
`macup <plugin> <command>` is the unified resource interface (PRD 5.1): the same list, install, update, add, and remove verbs across every backend, generated from each plugin's manifest so dispatch, help, and completions never need per-plugin edits.

## Requirements

### Generation

1. Subcommands are built from `manifest.capabilities` by a single factory: list, install, update, add, remove appear only when the plugin advertises them (xcode and system get no add/remove; `all` gets list, install, update).
2. Plugins with more than one subtype (brew) accept `--subtype`, plus `--cask` and `--formula` shortcuts, on list, install, update, add, and remove; conflicting or unknown subtype flags print an error and exit 1.

### list

3. `list` shows tracked packages by default, filtering the plugin's full listing to names in applist.yaml; `--all` shows everything installed; with nothing tracked it warns and falls back to showing all, with an `add` hint.
...
12. `add` and `remove` are config-only: they mutate the tracked list in applist.yaml (creating a timestamped backup) and never run the package manager.
13. Both report exactly what changed and what did not (`Added to brew.formulas: ...`, `Already tracked: ...`, `Not present: ...`) and suggest the natural next command; missing positionals print usage and exit 1.
...
- apps/cli/test/integration/commands/update-positionals.test.ts, apps/cli/test/regression/add-remove-sees-packages.test.ts, apps/cli/test/regression/subtype-arg.test.ts, apps/cli/test/regression/validate-missing-args-exits-nonzero.test.ts
