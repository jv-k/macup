...
- A filesystem failure other than a missing file (permissions, a directory at the path) still propagates from both `read()` and `load()`. It is the machine's failure rather than the file's, and wording it is not this change's job.
