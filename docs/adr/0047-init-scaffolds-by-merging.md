...
Scaffolding originally tracked everything installed, which for Homebrew meant the dependency closure: a real run on the author's machine tracked 364 packages, 263 of them brew formulas, most pulled in by other formulae. Filtering to what the user chose needs per-backend knowledge (`brew leaves`), a plugin-contract question rather than a scaffolder one, so it was tracked as #128 and settled in ADR 0051: a plugin that can tell a chosen install from a dependency declares `leaves()`, and `init` scaffolds from that.
...
**`macup init --prune` untracks, under the keys the scan covered, the names the scan did not find.** The scan now records those keys (`DetectionPlan.scanned`): every key whose listing succeeded this run, including one that came back empty. The record is per key, not per backend. A backend that was unavailable has no scanned keys at all, so a machine without `mas` cannot lose its App Store list. A listing that errored leaves its own key out and no other, so brew's formulas can be pruned on a run where its casks listing broke, and the casks are left alone. Entries under an unscanned key are never consulted. An empty listing over a covered key is a real answer, though, and everything tracked there is treated as stale. When no key was covered at all, `--prune` says so rather than leaving "nothing to write" to imply "nothing to prune".

**It has a confirmation of its own.** A tracked-but-not-installed entry can be intent rather than drift (CONTEXT.md, Tracked vs Installed), so the prune prints what it would untrack, by key, and asks. It asks regardless of what the applist held before, because unlike the merge there is always something to lose. Under a pipe it refuses and names `--force`, the same spelling the merge uses, and `--force` answers both questions at once: `--prune --force` is what a script says, and a user who adds `--force` for the merge has also said yes to the prune, which the help text for both flags spells out. Declining the prune leaves the merge's answer standing, so the two questions are independent.
