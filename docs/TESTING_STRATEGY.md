...
├── e2e/             # the compiled binary, spawned against a PATH of stub backends
└── fixtures/
...
The one layer that observes the packaged artifact rather than the sources. `binary-smoke.test.ts` spawns the bun-compiled binary (`apps/cli/dist/macup-darwin-<arch>`) as a child process with an environment built from nothing: `PATH` is a single directory of stub backends, `HOME` and `MACUP_CONFIG` point into a `mkdtemp()` sandbox, and nothing is imported from `src/`. It asserts exit codes, key stdout/stderr lines, and which stubs the binary actually called.

The stubs are generated by `stub-path.ts` from the same `test/fixtures/recordings/*.json` the plugin suites replay, so canned backend output has one home. Each stub is a `/bin/sh` script that matches its exact argv, logs every call, and exits 99 on an argv it has no recording for. The suite asserts that miss log is empty, so a binary that reaches a stub with the wrong arguments fails rather than passing on empty output.

Gating, because the binary is not a `pnpm test` prerequisite:

| `MACUP_E2E_BINARY` | binary present | result |
|---|---|---|
| unset | yes | runs against `dist/macup-darwin-<arch>` |
| unset | no | the file is skipped |
| `<path>` (relative to `apps/cli`) | yes | runs against that path |
| `<path>` | no | the file fails |
