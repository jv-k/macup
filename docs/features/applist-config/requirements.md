...
15. Bare `macup init` scans every available plugin, collects what each reports as installed, and merges those names into the applist under the key that plugin's `track` verb writes to (ADR 0047). Where a plugin can tell a chosen install from a dependency it declares `leaves()`, and the scan files those rather than the whole closure (ADR 0051). Homebrew formulas are the case today, and a plugin without the operation is scanned as before. It never replaces the file: pins, skip lists, and comments survive, and a second run adds nothing. An empty applist is written without a prompt; a populated one prompts, and under a non-TTY refuses with a pointer at `--force` rather than rewriting a config unattended. `--dry-run` prints the plan and writes nothing. An unavailable backend is reported and skipped, not fatal.
16. `macup init --prune` untracks, under the keys the scan covered, the names the scan did not find (#127, ADR 0047 amendment). It prints what it would untrack by key and asks its own confirmation. Under a non-TTY it refuses and points at `--force`. Keys of an unavailable or failed backend are never consulted. Pins and skip entries for a pruned name stay. `--dry-run --prune` names the keys it would touch without opening the store.
