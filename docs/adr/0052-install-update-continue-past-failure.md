...
4. Exit code is non-zero iff the run produced at least one `failed` package or a backend failed
   as a whole. A backend fails as a whole when its planning probe errored out before any ref could
   be selected, or when the `list()` probe verifying after the batch could not run. Either forces
   non-zero even when no package entry is `failed`, since the run cannot vouch for that backend's
   packages. `unavailable` alone does not force non-zero: ordinary `all` runs, unlike a bundle,
   name no targets, so a missing backend stays the environmental fact ADR 0033 and ADR 0037
   already treat it as, not a shortfall.
